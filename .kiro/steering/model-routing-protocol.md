...
| **2** | Light reasoning, well-known patterns. | **GLM-5** | Wiring a documented library API, straightforward struct/state definitions, low risk of subtle bugs. |
| **3** | Moderate engineering. Real decisions, established patterns. | **DeepSeek 3.2** | The workhorse tier — most tasks in this project land here. Strong reasoning-per-dollar. |
...
**Total CAT 5 tasks across the whole project: 3** — WO-1's ring buffer, WO-3's SIMD blendshape regression, WO-5's WebGPU viewport. **28 of the 36 total tasks are CAT 1-3 and never touch Claude at all** — Qwen3 Coder Next, GLM-5, and DeepSeek 3.2 carry the bulk of the build. Claude (Sonnet 5 then Opus 5) is confined to the 5 CAT 4 tasks plus the 3 CAT 5 tasks — 8 of 36, roughly 22% of the total task count, and typically the highest-value 22% to spend real reasoning on.
